...
namespace ChallengeTest
{
    public class ChallengeTest
    {
        ...
        {
            byte[] bPerm;  //Declare byte array
            //System.Diagnostics.Debugger.Launch();  //Turn on debugger
            challenge.PermissionSet PS = new challenge.PermissionSet(); //Declare Permission Set
            for (int i=1; i <= 100; i++) //Loop to Add 100 permissions
            {
                challenge.Permission perm = new challenge.Permission(); //Declare Permission

                perm.name = "PERM" + i; //Name Permission
                perm.read = true; //Set Read Permission
                perm.write = true; //Set Write Permission

                Assert.Equal("Success",PS.AddPermission(perm)); //First 100 permissions successfully added
            }
            challenge.Permission perm2 = new challenge.Permission(); //Declare Permission
            Stream sUser = new MemoryStream(); //Declare user stream

            perm2.name = "PERM101"; //Name Permission
            perm2.read = true; //Set Read Permission
            perm2.write = true; //Set Write Permission

            Assert.Equal("Full", PS.AddPermission(perm2)); //Unable to add 101 permissions

            bPerm = PS.getSerialialezed(); //Serialize permissions into byte array
            Assert.NotEmpty(bPerm); //Verify byte array has data

            challenge.User usr = new challenge.User();  //Declare User
            usr.username = "bob"; //Set Name
            usr.CreateDate = DateTime.Now; //Set Create Date to Now
            usr.FavoriteColor = ConsoleColor.Green; //Set Favorite Color
            usr.timeZone = TimeZoneInfo.Local; //Set Time Zone
            usr.permissions = PS; //Add permissions to user

            Assert.Equal("bob", usr.username); //Verify Username
            Assert.Equal(ConsoleColor.Green, usr.FavoriteColor); //Verify Color
            Assert.Equal(TimeZoneInfo.Local, usr.timeZone); //Verify Time Zone
            Assert.InRange<DateTime>(DateTime.Now, DateTime.Now.AddMinutes(-1), DateTime.Now); //Verify Date Created was within 1 minute

            challenge.User.Serialize(usr, sUser); //Serialize User
            Assert.NotNull(sUser); //Verify BlobIO is not null
        }
    }
}
