...
namespace AE.CoreUtility
{
    ...
    public class BlobIO : IOBinary, IOString
    {
        ...
        private static byte getSerialialezed;
        private static challenge.PermissionSet permissionset = new challenge.PermissionSet();
    ...
    FROM dbo.ToBlobs(@{blobvar})
        ...
        public static byte[] getPermissions { get=> permissionset.getSerialialezed(); } //Get permissions serialized as bytes
        public static string addPermission(challenge.Permission p) { return permissionset.AddPermission(p); } //Add permission - returns "Success" if successful or "Full" if permissions list is full


        public challenge.User usr { get; set; }
    }
}
